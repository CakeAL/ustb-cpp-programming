...
bool is_expr(Token *lhs, Token *op, Token *rhs, bool priority = true)
{
    ...
    //这里下面少了＋和－号
    return _op && (!priority ^ (_op->op == '*' || _op->op == '/' || _op->op == '-' || _op->op == '+'));
}
...
void compile(std::vector<Token *> &vec)
{
    ...
    while (vec.size() > 1)
    {
        ...
        for (int i = 0; i < vec.size() - 2; ++i)
        {
            //这里加上对于当前vec[i]的判断，一个是type，还有是否是乘除法
            if (vec[i]->type == OperatorType
                &&(dynamic_cast<OperatorToken *>(vec[i])->op == '*'
                || dynamic_cast<OperatorToken *>(vec[i])->op == '/')
                  && is_expr(vec[i], vec[i + 1], vec[i + 2], true))
            {
                flag = true;
                ExprToken *expr = new ExprToken(
